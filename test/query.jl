@testset "query.jl                      " begin
    ...
    for _ in 1:nmax_repeat_selected_query_tests
        for selected_precision in reverse(PRECISIONS)
            @info("Query (query_flux_raw) - Testing percision roundtrip for precision = $(selected_precision)...")
            #define data
            ...
            #write data
                ...
                @test rs == 204
            #read data
                rs2 = query_flux_raw(isettings,a_random_bucket_name,"myMeasurementWithPrecision$(selected_precision)",range=Dict("start"=>dt - Hour(1)),fields=["fieldKeyX"])
                rs3 = String(rs2)
